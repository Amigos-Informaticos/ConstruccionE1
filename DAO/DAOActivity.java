...
public class DAOActivity implements IDAOActivity {
	...
	public boolean create() throws CustomException {
		...
		assert this.activity.isComplete() : "Activity is not complete: DAOActivity.create()";
		String query =
			"INSERT INTO Actividad (idPracticante, titulo, descripcion, fechaInicio, fechaCierre) " +
				"VALUES ((SELECT idUsuario FROM Usuario WHERE correoElectronico = ?), " +
				"?, ?, (SELECT SYSDATE()), ?)";
		String[] values = {
			"edsonn1999@hotmail.com", //TODO WTF WHY?
			this.activity.getTitle(),
			this.activity.getDescription(),
			this.activity.getDeliveryDate()};
		//TODO What where you doing?
