...
public class DAOProject implements IDAOProject {
		...
		if (this.project.isComplete()) {
			if (!this.isRegistered() && !this.isActive()) {
				String query = "INSERT INTO Proyecto (nombre, metodologia, " +
					...
					"responsabilidades, estaActivo, area, responsable, idPeriodo, idOrganizacion)" +
					"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	public boolean isRegistered() {
		assert this.project != null : "Project is null: DAOProject.isRegistered()";
		assert this.project.getName() != null : "Project's name is null: DAOProject.isRegistered()";
	...
	public Project loadProject(String name) {
		...
		assert name != null : "Provided name is null: DAOProject.loadProject()";
		String query =
	...
	public boolean delete() throws CustomException {
		assert this.project != null : "Project is null: DAOProject.delete()";
		assert this.isRegistered() : "Project is not registered: DAOProject.delete()";
	...
	public boolean isActive() {
		assert this.project != null : "Project is null: DAOProject.isActive()";
		assert this.project.getName() != null : "Project's name is null: DAOProject.isActive()";
		assert this.isRegistered() : "Project is not registered: DAOProject.isActive()";
	...
	public boolean reactive() {
		assert this.project != null : "Project is null: DAOProject.reactive()";
		assert this.isRegistered() : "Project is not registered: DAOProject.reactive()";
		boolean reactivated = true;
		if (!this.isActive()) {
	...
	public boolean haveStudents() {
		...
		String query = "SELECT idProyecto FROM Asignacion WHERE idProyecto = ?";
		String[] values = { this.getId() };
		String[] names = { "idProyecto" };
		String[][] result = this.connection.select(query, values, names);
		if (!result[0][0].equals("")) {
