...
public class DAOAsignacion {
    ...
    public boolean asignarProyecto() throws SQLException {
        ...
        String query = "SELECT COUNT(idPracticante) AS TOTAL FROM Asignacion " +
                "WHERE idPracticante = ? AND idProyecto = ? AND estado = 1";
        ...
        if (resultados != null && resultados[0][0].equals("0")) {
            query = "INSERT INTO Asignacion (idPracticante, idProyecto, estado) VALUES (?, ?, 1)";
            valores = new String[]{
                    idPracticante,
                    idProyecto
            };
